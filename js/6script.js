...
const wordSelector = () => {
        ...
        let rand = sixwords[Math.floor(Math.random()*sixwords.length)]; //this will select a random word from our words.js file and store as Object {word: "example"}
        console.log(rand); //checking the word is selected
       
        
        let word = rand.sixword.split(""); //splits the chosen string into characters, is kept as Array(7) {'e','x','a','m','p','l','e'}
    ...
    function handleClick() {
        ...
        let rand = sixwords[Math.floor(Math.random()*sixwords.length)];
        console.log(rand);

        let word = rand.sixword.split("");
        console.log(word);
